...
resource "aws_launch_configuration" "level-launchconfig" {
  name          = "level-launchconfig"
...
resource "aws_autoscaling_group" "levelup-autoscaling" {
  ...
  launch_configuration      = aws_launch_configuration.level-launchconfig.name
  min_size                  = 1
  max_size                  = 2
...
resource "aws_cloudwatch_metric_alarm" "levelup-cpu-alarm-scaledown" {
  ...
  alarm_description   = "Alarm once CPU Usage "
